  ...
  useEffect(() => {
    const fetchTotalManagers = async () => {
      try {
        ...
        const response = await api.get("/managers/count/total", {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
        setTotalManagers(response.data.total_managers);
      } catch (error) {
    ...
    <div className="flex flex-col col-span-full sm:col-span-6 xl:col-span-4 bg-white dark:bg-gray-800 shadow-xs rounded-xl">
      <div className="px-5 pt-5">
        ...
        <div className="flex items-start">
          ...
          <div className="text-sm font-medium text-blue-600 dark:text-blue-400 px-1.5 bg-blue-100 dark:bg-blue-900/30 rounded-full">
            Total
          </div>
        </div>
      </div>
