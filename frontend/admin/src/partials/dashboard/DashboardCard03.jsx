...
function DashboardCard03() {
  ...
  useEffect(() => {
    const fetchInactiveManagers = async () => {
      try {
        const token = localStorage.getItem("adminToken");
        const response = await api.get("/managers/count/inactive", {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
        setInactiveManagers(response.data.inactive_managers);
      } catch (error) {
        console.error("Error fetching inactive managers:", error);
    ...
    <div className="flex flex-col col-span-full sm:col-span-6 xl:col-span-4 bg-white dark:bg-gray-800 shadow-xs rounded-xl">
      <div className="px-5 pt-5">
        ...
        <div className="flex items-start">
          ...
          <div className="text-sm font-medium text-red-600 dark:text-red-400 px-1.5 bg-red-100 dark:bg-red-900/30 rounded-full">
            Inactive
          </div>
        </div>
      </div>
